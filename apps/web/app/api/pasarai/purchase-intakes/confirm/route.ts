...
export async function POST(request: Request) {
  ...
  if (requestErrors.length > 0) {
    return Response.json(
      { error: "Invalid purchase confirmation.", details: requestErrors },
      { status: 400 }
    );
  }
  ...
  try {
    response = await fetch(
      new URL("/api/v1/purchase-intakes/confirm", apiBaseUrl),
      {
        method: "POST",
        cache: "no-store",
        headers: {
  ...
  if (response.ok && validateContract("costs.response", body).length > 0) {
    return Response.json(
