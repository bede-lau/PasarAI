...
export async function GET(request: Request) {
  ...
  const url = new URL("/api/v1/analytics/overview", configured.apiBaseUrl);
  ...
  try {
    ...
    if (
      response.ok
      && validateContract("analytics-overview.response", body).length
    ) {
      return Response.json(
        { error: "PasarAI API returned an invalid analytics overview." },
        { status: 502 }
      );
    }
    ...
    return Response.json(
      { error: "Analytics overview is temporarily unreachable." },
      { status: 503 }
    );
  }
}
